...
class RedelmeierFreePolyominoGenerator {
	...
	/*
	 * Normalize le polyomino en input en le translatant de sorte à ce qu'aucune
	 * abscisse / ordonnée ne soit négative
	 */
	...
	/*
	 * Même fonction sur le principe, mais génère à partir de l'algorithme de
	 * Redelmeier.
	 */
	public static ArrayList<short[]> generateFreePolyominoes(int n) {
		...
		// Un comparateur pour les arrays : c'est simplement un ordre
		// lexicographique
	...
	public static ArrayList<ArrayList<Pair>> generateFreePolyominoesAsCoord(int n) {
		return generateFreePolyominoes(n).stream().map((short[] l) -> codage2ListOfPairs(n, l))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	public static ArrayList<short[]> classeEquivalence(final short[] pol) {
		int n = pol.length;
		ArrayList<short[]> ret = new ArrayList<short[]>(7);
	...
	public static ArrayList<ColoredPolygon> generateFreePolyominoesAsColoredPolygon(int n) {
		...
		return generateFreePolyominoes(n).stream().map((short[] l) -> codage2ColoredPolygon(n, l))
				.collect(Collectors.toCollection(ArrayList::new));
	}
